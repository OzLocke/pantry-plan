...
class UserInput:
    ...
    def input_pantry_entries(self):
        ...
        while more == "y":
            ...
            # Add to pantry list
            self.pantry_instance.update_pantry(item, unit, quantity, self.verbose)
            # Check for continue
...
class Pantry:
    ...
    def update_pantry(self, item, unit, quantity, verbose):
        ...
        if any(entry["item"] == item for entry in self.pantry):
            ...
            if self.pantry[item_index]["unit"] == unit:
                ...
                if verbose: print(f"SYSTEM: Creating new entry for duplicate {item} with new unit of {unit} and quantity of {quantity}")
                new_entry = {"item": item, "unit": unit, "quantity": quantity}
                self.pantry.append(new_entry)
        else:
            # Build dictionary and add to pantry list
            if verbose: print(f"SYSTEM: Creating new entry for {item} with uinit of {unit} and quantity of {quantity}")
            new_entry = {"item": item, "unit": unit, "quantity": quantity}
            self.pantry.append(new_entry)
